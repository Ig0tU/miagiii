 ...
 */
export function customEventHelpers<TDetail>(eventName: string): [dispatchEvent: (target: EventTarget, detail: TDetail) => void, installListener: (target: EventTarget, listener: (detail: TDetail) => void) => () => void] {
  ...
  const installListener = (target: EventTarget, detailListener: (detail: TDetail) => void) => {
