...
export function LocalAISourceSetup(props: { sourceId: DModelSourceId }) {
  ...
  const { source, sourceHasLLMs, access, updateSetup } = useSourceSetup(props.sourceId, ModelVendorLocalAI);
  ...
  const userHostValid = Boolean(localAIHost) && localAIHostSchema.safeParse(localAIHost).success;
  const userHostError = !!localAIHost && !userHostValid;
  const shallFetchSucceed = userHostValid || backendHasHost;

  // fetch models - the OpenAI way
  const { isFetching, refetch, isError, error } = useLlmUpdateModels(shallFetchSucceed && !sourceHasLLMs, source);
