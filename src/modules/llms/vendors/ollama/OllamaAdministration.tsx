...
export function OllamaAdministration(props: Props) {
  // state
  ...
  // external state
  const { data: pullableData } = apiQuery.llmOllama.adminListPullable.useQuery(
    { access: props.access },
    {
      staleTime: 1000 * 60,
  ...
  // reset mutations on unmount
  React.useEffect(() => {
    return () => {
      pullReset();
      deleteReset();
    };
  }, [pullReset, deleteReset]);
  ...
  const handleModelPull = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    deleteReset();
    if (modelName) pullMutate({ access: props.access, name: modelName + (modelTag ? ':' + modelTag : '') });
  };

  const handleModelDelete = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    pullReset();
    if (modelName) deleteMutate({ access: props.access, name: modelName + (modelTag ? ':' + modelTag : '') });
  };

  return (
    <GoodModal
      title='Ollama Administration'
      dividers
      open
              ...
              >
                {pullable.map((p) => (
                  <Option
                    ...
                    select
                  >
                    {p.isNew === true && <Chip size='sm' variant='solid'>NEW</Chip>}
                    {p.label}
                    {sortByPulls && ` (${p.pulls.toLocaleString()})`}
                  </Option>
                ))}
              </Select>
              ...
              <Input
                ...
                endDecorator={
                  !!modelName && (
                    <IconButton
                      component={Link}
                      href={`https://ollama.ai/library/${modelName}`}
                      target='_blank'
                    >
                      <LaunchIcon />
                    </IconButton>
