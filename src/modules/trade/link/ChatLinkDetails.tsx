...
import { Link as RouterLink } from '~/common/components/Link';
...
interface ChatLinkDetailsProps {
  open: boolean;
  onClose: () => void;
...
export function ChatLinkDetails(props: ChatLinkDetailsProps) {
  // state
  ...
  const handleDeleteNow = () => setConfirmDeletion(true);
  const handleDeleteCancelled = () => setConfirmDeletion(false);

  const handleConfirmedDeletion = async () => {
    const result: StorageDeleteSchema = await apiAsyncNode.trade.storageDelete.mutate({
      objectId,
      deletionKey,
    });
    setDeletionResponse(result);
    if (result.type === 'success') forgetChatLinkItem(objectId);
  ...
  return (
    <GoodModal
      title={isDeleted ? '🗑️ Link deleted' : '🔗 Link created'}
      strongerTitle={isDeleted}
      dividers={!isDeleted}
      open
      onClose={props.onClose}
    >
      ...
      <ConfirmationModal
        ...
        onPositive={handleConfirmedDeletion}
