...
import { LLMAttachment, LLMConverter } from './useLLMAttachments';
...
export function AttachmentMenu(props: {
  ...
  const { id: aId, input: aInput, converters: aConverters, converterIdx: aConverterIdx, outputs: aOutputs } = attachment;
  ...
  const renderConverters = () => {
    if (isUnconvertible) return null;

    return (
      <>
        <ListItemDecorator>
          <Typography level='body-md'>Attach as:</Typography>
        </ListItemDecorator>
        {aConverters.map((converter: LLMConverter, idx: number) => (
          <MenuItem
            key={'c-' + converter.id}
          ...
          >
            ...
            {converter.unsupported ? (
              <Box>
                Unsupported 🤔
                <Typography level='body-xs'>{converter.name}</Typography>
              </Box>
            ) : (
              converter.name
            )}
      ...
      {DEBUG_ATTACHMENTS && !!aInput && (
        <>
          <MenuItem
            ...
            disabled={!isOutputTextInlineable}
          >
            <ListItemDecorator>
              <ContentCopyIcon />
            </ListItemDecorator>
