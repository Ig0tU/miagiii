...
import {
  deleteSimplePersona,
  deleteSimplePersonas,
  useSimplePersonas,
} from '../store-app-personas';
    ...
    <>
      ...
      >
        {hasPersonas && !selectMode && (
          <Tooltip title={selectMode ? 'Done' : 'Select'}>
            <IconButton
              onClick={selectMode ? handleSelectionClose : () => setSelectMode(true)}
            >
      ...
      >
        {selectMode ? (
          ...
          <Button
            variant={
              selectedSimplePersonaId ? 'plain' : 'soft'
            }
            onClick={handleSimplePersonaUnselect}
            sx={{
              m: 2,
        ...
        <Box sx={{ flex: 1, overflowY: 'auto' }}>
          {simplePersonas.map((item) => (
            <CreatorDrawerItem
              key={item.id}
              item={item}
              isActive={item.id === selectedSimplePersonaId}
              ...
              onClick={(event) => {
                ...
                if (selectMode) {
                  ...
                  setSelectedSimplePersonaId(item.id);
                }
              }}
